...
For the current buyer-first walkthrough, use [the September 14 two-minute recording script](submission/demo-script.md). It compares two providers under the same brief, shows the existing verified Speechmatics v3 and AssemblyAI v4 findings, explains required setup, and exports the comparison. Do not sign a new transaction just to record the walkthrough. The historical payment experiment below is a separate optional appendix. Run [the uncoached usability task](submission/usability-check.md) before recording; neither a participant result nor a finished video is claimed.

## Historical three-minute walkthrough: the user's wallet run
