...
for(const version of [20,22])test(`Next intent and pinned v${version} saved assessment remain readable in index and report`,async()=>{
  const {source,protocol}=DECISION_FORMATS[version];
  ...
  Object.assign(entry.review,{chain_id:61997,source_sha256:source,protocol_fee_wei:'123'});
  Object.assign(row.session.receipt,{chain_id:61997,source_sha256:source,protocol_fee_deposit_wei:'123'});
  delete s.results;
  Object.assign(s,{version,kind:'provider-review-decisions-candidate',release_cleared:false,protocol_sha256:protocol,
    assessment:{kind:'local-evidence-decisions-experiment',schema_version:version,release_cleared:false,evidence_sha256:row.digest,
  ...
  assert.equal(index.unavailable,false);assert.equal(index.entries[0].report.version,version);
