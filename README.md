...
The working product compares 17 plans from 13 providers across transcription, speech generation and text generation. Comparison, evidence capture, saved options and decision exports do not require a wallet. For transcription and speech generation, a user can separately approve a GenLayer Studio Next assessment of captured public evidence. GenLayer supplies the on-chain semantic judgment; catalog price calculations and the interface do not substitute an off-chain verdict. The assessment checks documented claims, not provider behavior, account settings or legal compliance.

**Release boundary:** New text-generation assessments are not enabled in the hosted writer. The v26 contract and reader are experimental: one frozen Speechmatics case completed, but this does not establish general provider accuracy or clear a broader release. The separate purchase pages are test-token demonstrations, not checkout for catalog providers. Studio Next is a development network, so this is a Builder-program MVP rather than a claim of audited or durable production infrastructure. The experiment history below is preserved for transparency; start with the [builder overview](submission/BUILDER-PROJECT.md) for the shortest review path.
