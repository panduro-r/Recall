...
The working product compares 17 plans from 13 providers across transcription, speech generation and text generation. Comparison, evidence capture, saved options and decision exports do not require a wallet. A user can separately approve a GenLayer Studio Next assessment of captured public evidence for audio plans and four selected text plans. GenLayer supplies the on-chain semantic judgment; catalog price calculations and the interface do not substitute an off-chain verdict. The assessment checks documented claims, not provider behavior, account settings or legal compliance.

**Release boundary:** The opt-in v25 text writer is a validation candidate on Studio Next, not a production-audited service. Only OpenAI, Mistral AI, DeepSeek and Anthropic text plans are enabled; Google remains evidence-only. Results are labelled experimental and never promoted to a confirmed provider fit solely because consensus finalized. The separate purchase pages are test-token demonstrations, not checkout for catalog providers. Studio Next is a development network, so this is a Builder-program MVP rather than durable production infrastructure. The experiment history below is preserved for transparency; start with the [builder overview](submission/BUILDER-PROJECT.md) for the shortest review path.

## Current build status — September 25

The opt-in v25 text writer uses the exact source validated in saved controls and OpenAI, Mistral and DeepSeek cases. A new Anthropic Haiku test with official model, Messages API and explicit commercial input/output policy evidence finalized successfully and its exact citations were manually checked; see the [transaction record](submission/studio-next-v25-anthropic-explicit-policy-2026-09-25.json). This is still a candidate, not a general accuracy guarantee. Google has no new writer access. Historical v26 results remain read-only, including the [Speechmatics conflict case](submission/studio-next-v26-speechmatics-2026-09-24.json). A failed earlier Anthropic attempt and the stronger later capture are separate, preserved transactions; neither was automatically retried.
