...
Recall helps developers compare API services against cost and data-use requirements, then inspect captured evidence and GenLayer assessments before choosing a provider.

**Try the MVP:** [Live app](https://recall-navy-phi.vercel.app/compare) · [Builder overview and demo path](submission/BUILDER-PROJECT.md) · [Source](https://github.com/panduro-r/Recall)

The working product compares 17 plans from 13 providers across transcription, speech generation and text generation. Comparison, evidence capture, saved options and decision exports do not require a wallet. A user can separately approve a GenLayer Studio Next assessment of captured public evidence for audio plans and five selected text plans. GenLayer supplies the on-chain semantic judgment; catalog price calculations and the interface do not substitute an off-chain verdict. The assessment checks documented claims, not provider behavior, account settings or legal compliance.

**Release boundary:** The opt-in v25 text writer is a validation candidate on Studio Next, not a production-audited service. OpenAI, Mistral AI, DeepSeek, Anthropic and Google text plans are enabled for an optional assessment of captured public terms. Google's paid-service data-use finding is conditional on an actively billed Cloud project, which Recall cannot verify. Results are labelled experimental and never promoted to a confirmed provider fit solely because consensus finalized. The separate purchase pages are test-token demonstrations, not checkout for catalog providers. Studio Next is a development network, so this is a Builder-program MVP rather than durable production infrastructure. The experiment history below is preserved for transparency; start with the [builder overview](submission/BUILDER-PROJECT.md) for the shortest review path.

## Current build status — September 25

The opt-in v25 text writer uses the exact source validated in saved controls and selected provider cases. The [Anthropic Haiku test](submission/studio-next-v25-anthropic-explicit-policy-2026-09-25.json) and a separate [Google paid-service test](submission/studio-next-v25-google-paid-2026-09-25.json) finalized successfully; their exact citations were manually checked. The Google finding concerns the documented paid API tier, not verification of a user's billing setup. This is still a candidate, not a general accuracy guarantee. Historical v26 results remain read-only, including the [Speechmatics conflict case](submission/studio-next-v26-speechmatics-2026-09-24.json). Failed earlier attempts and stronger later captures remain separate, preserved transactions; no attempt was automatically retried.
...
Next uses test-GEN protocol fees even when EVM gas is zero. The wallet confirmation separates the bounded protocol deposit from zero provider payment. Audio migration and the later text candidate are distinct: the former is the audio baseline; the latter remains experimental. Comparison, evidence capture and existing records remain available.

## Compare before committing

The catalog has **17 plans across 13 providers in three categories**: nine transcription plans, three speech-generation plans and five text-generation plans. The text providers are OpenAI, Anthropic, Google, Mistral AI and DeepSeek. Every text plan supports wallet-free comparison, saving, evidence capture and exports. All five have an optional, explicitly experimental GenLayer assessment. Google still requires confirmation of an actively billed API project before its paid-service terms apply. See [text-category scope and QA](tests/TEXT-CATEGORY-QA-2026-09-15.md).
...
For the practical use case, honest scope and category-expansion plan, read [the product pitch](submission/pitch.md). Three categories support comparison, evidence capture and optional Studio Next assessments; the v25 text path is explicitly experimental. Storage and other categories remain proposals. Release acceptance requires production checks; broader reliability evaluation and [an uncoached user test](submission/usability-check.md) also remain necessary. Historical wallet allowances, including the separate five-test v8 allowance, are consumed; none is automatically renewed.
...
The earlier multi-service experiment remains at `/recorded`; a fresh browser-wallet test is at `/purchase`. The contract source is unchanged. For Vercel import, use **Other** with root directory **`deploy`**. See [hosting instructions](hosting/README.md) for exact settings and post-deployment checks. The live app is [recall-navy-phi.vercel.app](https://recall-navy-phi.vercel.app/); verify the current deployment after future changes.
