...
export function buildComparisonReport(catalog,input,ids,index,now=Date.now()){
  ...
  const options=ids.map(id=>{
    ...
    return {id,name:plan.name,plan:plan.plan,result,catalogDate:date,
      price:plan.pricing==='from'?`Quote needed · ${usd(result.estimate)} from-rate calculation`:plan.pricing==='estimated'?`≈ ${usd(result.estimate)} / month · approximate token-based cost`:`${usd(result.estimate)} / month · ${result.uncertainPrice?'illustrative base cost only':'estimated usage cost'}`,
      priceNote:plan.priceNote,training:plan.trainingNote,speakers:!req.speakers?'Not selected; excluded from this estimate.':result.labelsUnknown?'Availability and cost need confirmation.':plan.diarization===0?'Included with no separate per-hour surcharge in the catalog.':`${usd(plan.diarization)} additional per audio hour, included in this estimate.`,
  ...
  return {generatedAt:new Date(now).toISOString(),requirements:req,options,summary:{headline:options.length===1?'Review this option before choosing':headline,difference},
    scope:'English, pre-recorded, single-channel transcription API.',
    ...
    next:'Confirm the selected configuration and account data settings, then test representative, non-sensitive audio before committing. Purchase separately with the provider; Recall has no provider checkout integration.'};
}
...
<div class="requirements"><span>${e(r.hours.toLocaleString('en-US'))} audio hours / month</span><span>${e(usd(r.budget))} monthly budget</span><span>${r.noTraining?'No model training required':'No training restriction selected'}</span><span>${r.speakers?'Speaker labels required':'Speaker labels not selected'}</span></div>
...
<h2>Cost and configuration</h2><div class="table-wrap"><table><thead><tr><th scope="col">Compare</th>${report.options.map(p=>`<th scope="col">${e(p.name)}<br><small>${e(p.plan)}</small></th>`).join('')}</tr></thead><tbody>${row('Monthly estimate',report.options.map(p=>p.price))}${row('Budget',report.options.map(p=>p.result.budgetLabel))}${row('Data-use policy',report.options.map(p=>p.training))}${row('Speaker labels',report.options.map(p=>p.speakers))}${row('Catalog reviewed',report.options.map(p=>p.catalogDate+(p.result.stale?' · Out of date':'')))}</tbody></table></div>
<h2>Before you choose</h2>${report.options.map(p=>`<section class="option"><h3>${e(p.name)} · ${e(p.plan)}</h3><p>${e(p.priceNote)}</p>${p.questions.length?list(p.questions):'<p>No additional catalog condition was flagged for these inputs. Confirm account settings and test your audio.</p>'}<p class="muted">Catalog sources · reviewed ${e(p.catalogDate)}${p.result.stale?' (out of date)':''}</p><ul>${p.sources.map(s=>`<li>${link(s.url,s.label)}</li>`).join('')}</ul></section>`).join('')}
