import {categoryOf,categoryMatches,scopeFor,workload,extraCondition,priceText} from './service-categories.js';
...
<h2>Cost and configuration</h2><div class="table-wrap"><table><thead><tr><th scope="col">Compare</th>${report.options.map(p=>`<th scope="col">${e(p.name)}<br><small>${e(p.plan)}</small></th>`).join('')}</tr></thead><tbody>${row('Monthly estimate',report.options.map(p=>p.price))}${row('Budget',report.options.map(p=>p.result.budgetLabel))}${row('Data-use policy',report.options.map(p=>p.training))}${row(categoryOf(r)==='speech'?'Streaming audio':'Speaker labels',report.options.map(p=>p.speakers))}${row('Catalog reviewed',report.options.map(p=>p.catalogDate+(p.result.stale?' · Out of date':'')))}</tbody></table></div>
