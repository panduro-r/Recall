...
function renderResults() {
  ...
  $('#results').replaceChildren(...rows.map(({plan,result})=>{
    ...
    const sourceLabel = plan.training === 'excluded' && !result.paidTierRequired ? '✓' : '◦';
    return el('article',{class:'plan-card','aria-label':`${plan.name} ${plan.plan}`},
      ...
      el('div',{class:'plan-condition'},el('span',{class:'condition-icon','aria-hidden':'true'},sourceLabel),el('div',{},el('strong',{},plan.trainingLabel),el('p',{},result.paidTierRequired ? 'Only applies to a project with active paid billing. Verify that setup before sending customer data.' : plan.training === 'excluded' ? 'Documented for this configuration. Check account settings and retention before use.' : plan.training === 'default-training' ? 'Does not meet a no-training condition by default.' : plan.training==='unknown'?'An applicable no-training commitment has not been established.': 'Configuration and any price impact must be confirmed.'))),
